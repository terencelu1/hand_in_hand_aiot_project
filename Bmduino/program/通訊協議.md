...
**格式**：
```
WORKING,FINAL,物體溫度,環境溫度,心率,血氧
```

**範例**：
```
WORKING,FINAL,25.50,23.20,72,98
```
    ...
    while True:
        if ser.in_waiting > 0:
            ...
            if line.startswith('STANDBY,'):
                ...
                # 解析工作模式最終數據
                parts = line.split(',')
                object_temp = float(parts[2])
                ambient_temp = float(parts[3])
                heart_rate = int(parts[4])
                spo2 = int(parts[5])
                
                print(f"測量完成 - 物體溫度: {object_temp}°C, 環境溫度: {ambient_temp}°C")
                print(f"  心率: {heart_rate} BPM, 血氧: {spo2}%")
...
if __name__ == "__main__":
    ...
    def on_working_final(data):
        print(f"測量完成: 心率={data['heart_rate']} BPM, 血氧={data['spo2']}%")
...
| 工作完成 | `WORKING,FINAL,溫度,溫度,心率,血氧` | 測量完成 |
